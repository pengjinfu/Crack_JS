...
function d(a, b, c) {
    null != a && ("number" == typeof a ? this.fromNumber(a, b, c) : null == b && "string" != typeof a ? this.fromString(a,
        256) : this.fromString(a, b))
}
...
function n(a, b) {
    ...
    for (var e = a.length, f = !1, g = 0; --e >= 0;) {
        ...
        0 > h ? "-" == a.charAt(e) && (f = !0) : (f = !1, 0 == g ? this[this.t++] = h : g + c > this.DB ? (this[this.t - 1] |=
            (h & (1 << this.DB - g) - 1) << g, this[this.t++] = h >> this.DB - g) : this[this.t - 1] |= h << g, g += c, g >=
        this.DB && (g -= this.DB))
    }
    8 == c && 0 != (128 & a[0]) && (this.s = -1, g > 0 && (this[this.t - 1] |= (1 << this.DB - g) - 1 << g)), this.clamp(),
...
function p(a) {
    ...
    if (g-- > 0)
        for (h < this.DB && (c = this[g] >> h) > 0 && (e = !0, f = i(c)); g >= 0;)
            b > h ? (c = (this[g] & (1 << h) - 1) << b - h, c |= this[--g] >> (h += this.DB - b)) : (c = this[g] >> (h -= b) & d,
            0 >= h && (h += this.DB, --g)), c > 0 && (e = !0), e && (f += i(c));
    return e ? f : "0"
}
    ...
    if (!(f.t <= 0)) {
        ...
        if (0 != m) {
            var n = m * (1 << this.F1) + (l > 1 ? h[l - 2] >> this.F2 : 0),
                ...
                s = r - l,
                u = null == b ? e() : b;
            for (h.dlShiftTo(s, u), c.compareTo(u) >= 0 && (c[c.t++] = 1, c.subTo(u, c)), d.ONE.dlShiftTo(l, u), u.subTo(h, h); h
                .t < l;)
                h[h.t++] = 0;
            for (; --s >= 0;) {
                var v = c[--r] == m ? this.DM : Math.floor(c[r] * o + (c[r - 1] + q) * p);
                if ((c[r] += h.am(0, v, c, s, 0, l)) < v)
                    for (h.dlShiftTo(s, u), c.subTo(u, c); c[r] < --v;)
                        c.subTo(u, c)
            }
            null != b && (c.drShiftTo(l, b), i != j && d.ZERO.subTo(b, b)), c.t = l, c.clamp(), k > 0 && c.rShiftTo(k, c), 0 > i &&
            d.ZERO.subTo(c, c)
        }
    }
}
...
function M(a) {
    var b = e();
    return a.abs().dlShiftTo(this.m.t, b), b.divRemTo(this.m, null, b), a.s < 0 && b.compareTo(d.ZERO) > 0 && this.m.subTo(
        b, b), b
}
...
function Y(a) {
    qb[rb++] ^= 255 & a, qb[rb++] ^= a >> 8 & 255, qb[rb++] ^= a >> 16 & 255, qb[rb++] ^= a >> 24 & 255, rb >= sb && (rb -=
        sb)
}
...
function cb(a, b) {
    ...
    for (var c = new Array, e = a.length - 1; e >= 0 && b > 0;) {
        ...
        128 > f ? c[--b] = f : f > 127 && 2048 > f ? (c[--b] = 63 & f | 128, c[--b] = f >> 6 | 192) : (c[--b] = 63 & f | 128,
...
function eb(a, b) {
    null != a && null != b && a.length > 0 && b.length > 0 ? (this.n = bb(a, 16), this.e = parseInt(b, 16)) : alert(
        "网络异常，请点击登录重试")
}
...
jb && "Microsoft Internet Explorer" == navigator.appName ? (d.prototype.am = g, hb = 30) : jb && "Netscape" !=
navigator.appName ? (d.prototype.am = f, hb = 26) : (d.prototype.am = h, hb = 28), d.prototype.DB = hb, d.prototype.DM =
    (1 << hb) - 1, d.prototype.DV = 1 << hb;
var kb = 52;
...
for (lb = "A".charCodeAt(0), mb = 10; 36 > mb; ++mb)
    ob[lb++] = mb;
E.prototype.convert = F, E.prototype.revert = G, E.prototype.reduce = H, E.prototype.mulTo = I, E.prototype.sqrTo = J,
    L.prototype.convert = M, L.prototype.revert = N, L.prototype.reduce = O, L.prototype.mulTo = Q, L.prototype.sqrTo = P,
    d.prototype.copyTo = k, d.prototype.fromInt = l, d.prototype.fromString = n, d.prototype.clamp = o, d.prototype.dlShiftTo =
    v, d.prototype.drShiftTo = w, d.prototype.lShiftTo = x, d.prototype.rShiftTo = y, d.prototype.subTo = z, d.prototype.multiplyTo =
    A, d.prototype.squareTo = B, d.prototype.divRemTo = C, d.prototype.invDigit = K, d.prototype.isEven = R, d.prototype.exp =
    S, d.prototype.toString = p, d.prototype.negate = q, d.prototype.abs = r, d.prototype.compareTo = s, d.prototype.bitLength =
    u, d.prototype.mod = D, d.prototype.modPowInt = T, d.ZERO = m(0), d.ONE = m(1), U.prototype.init = V, U.prototype.next =
    W;
var pb, qb, rb, sb = 256;
if (null == qb) {
    qb = new Array, rb = 0;
...
// 获取用户加密
function getUser(user, a) {
    // a 是通过请求：https://passport.migu.cn/password/publickey 得来的
    // a = {
    // 	"status": 2000,
    // 	"message": "",
    // 	"header": {},
...
// 获取fingerPrint加密,获取fingerPrintDetail加密
...
function rsaFingerprint(a, b) {
    ...
    var c = '"{"user_agent":"Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3314.0 Safar","language":"zh-CN","color_depth":"24","pixel_ratio":"1","hardware_concurrency":"4","resolution":"1680,1050","available_resolution":"1680,1010","timezone_offset":"-480","session_storage":"1","local_storage":"1","indexed_db":"1","open_database":"1","cpu_class":"unknown","navigator_platform":"Win32","do_not_track":"unknown","regular_plugins":"360SoftMgrPlugin::360SoftMgrPlugin::application/360softmgrplugin~dll,360安全卫士 快速登录::360安全卫士 快速登录::app","webgl_vendor":"Google Inc.~ANGLE (NVIDIA GeForce 210  Direct3D11 vs_4_1 ps_4_1)","adblock":"false","has_lied_languages":"false","has_lied_resolution":"false","has_lied_os":"false","has_lied_browser":"false","touch_support":"0,false,false","js_fonts":"Arial,Arial Black,Arial Narrow,Calibri,Cambria,Cambria Math,Comic Sans MS,Consolas,Courier,Courier N"}"'
        // , d = $.fingerprint.result
        , d = 'e1c7a33d747a5c51c27466845cbb78a5'
        , e = c.length
        , f = ""
        , g = new RSAKey;
    g.setPublic(a, b);
