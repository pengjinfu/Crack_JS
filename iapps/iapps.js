...
function bnToString(t) {
    ...
    if (s-- > 0) for (h < this.DB && (r = this[s] >> h) > 0 && (n = !0, e = int2char(r)); s >= 0;)
    i > h ? (r = (this[s] & (1 << h) - 1) << i - h, r |= this[--s] >> (h += this.DB - i)) : (r = this[s] >> (h -= i) & o, 0 >= h && (h += this.DB, --s)), r > 0 && (n = !0), n && (e += int2char(r));
    return n ? e : "0"
}
...
function bnpLShiftTo(t, i) {
    var r, o = t % this.DB,
        ...
        h = this.s << o & this.DM;
    for (r = this.t - 1; r >= 0; --r)
    i[r + s + 1] = this[r] >> n | h, h = (this[r] & e) << o;
    for (r = s - 1; r >= 0; --r)
    i[r] = 0;
...
function bnpRShiftTo(t, i) {
    ...
    i[s - r - 1] |= (this[s] & e) << n, i[s - r] = this[s] >> o;
    ...
    if (!(o.t <= 0)) {
        ...
        if (0 != f) {
            var u = f * (1 << this.F1) + (a > 1 ? e[a - 2] >> this.F2 : 0),
                ...
                b = v - a,
                l = null == i ? nbi() : i;
            for (e.dlShiftTo(b, l), r.compareTo(l) >= 0 && (r[r.t++] = 1, r.subTo(l, r)), BigInteger.ONE.dlShiftTo(a, l), l.subTo(e, e); e.t < a;)
            e[e.t++] = 0;
            for (; --b >= 0;) {
                var T = r[--v] == f ? this.DM : Math.floor(r[v] * g + (r[v - 1] + c) * m);
                if ((r[v] += e.am(0, T, r, b, 0, a)) < T) for (e.dlShiftTo(b, l), r.subTo(l, r); r[v] < --T;)
                r.subTo(l, r)
            }
...
function montReduce(t) {
    ...
    for (var i = 0; i < this.m.t; ++i) {
        ...
        for (r = i + this.m.t, t[r] += this.m.am(0, o, t, i, 0, this.m.t); t[r] >= t.DV;)
        t[r] -= t.DV, t[++r]++
    }
...
function ARC4init(i) {
    ...
    this.S[t] = t;
    for (s = 0, t = 0; 256 > t; ++t)
    s = s + this.S[t] + i[t % i.length] & 255, h = this.S[t], this.S[t] = this.S[s], this.S[s] = h;
    this.i = 0, this.j = 0
}
...
function hex2b64(r) {
    ...
    a = parseInt(r.substring(t, t + 3), 16), n += b64map.charAt(a >> 6) + b64map.charAt(63 & a);
...
function b64tohex(r) {
    ...
    v = b64map.indexOf(r.charAt(t)), v < 0 || (0 == h ? (n += int2char(v >> 2), a = 3 & v, h = 1) : 1 == h ? (n += int2char(a << 2 | v >> 4), a = 15 & v, h = 2) : 2 == h ? (n += int2char(a), n += int2char(v >> 2), a = 3 & v, h = 3) : (n += int2char(a << 2 | v >> 4), n += int2char(15 & v), h = 0));
